...
static int cpufreq_governor_batterymax(struct cpufreq_policy *policy,
		unsigned int event);

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_BATTERYMAX
static
#endif
struct cpufreq_governor cpufreq_gov_batterymax = {
	.name = "batterymax",
	.governor = cpufreq_governor_batterymax,
...
static int cpufreq_governor_batterymax(struct cpufreq_policy *new_policy,
		unsigned int event)
{
...
static int __init cpufreq_smartmax_init(void)
{
	...
	return cpufreq_register_governor(&cpufreq_gov_batterymax);
}
...
{
	cpufreq_unregister_governor(&cpufreq_gov_batterymax);
}
